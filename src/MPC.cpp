...
vector<double> MPC::Solve(Eigen::VectorXd x0, Eigen::VectorXd coeffs) {
  ...
  vector<double> solve_result;

  solve_result.push_back(solution.x[delta_start]);
  solve_result.push_back(solution.x[a_start]);

  for (int i = 0; i < N-1; i++) {
    solve_result.push_back(solution.x[x_start + i + 1]);
    solve_result.push_back(solution.x[y_start + i + 1]);
  }
